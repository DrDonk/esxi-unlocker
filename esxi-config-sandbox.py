...
def main():
    vmsvc = '<vmsvc>\n'
    useVMXSandbox = '<useVmxSandbox>false</useVmxSandbox>\n'
    enforceVmxSandbox = '<enforceVmxSandbox>false</enforceVmxSandbox>\n'
    ...
    with open('/etc/vmware/hostd/config.xml', 'r+') as f:
        ...
        for line in data:
            ...
            if testline(line, useVMXSandbox):
                sandboxindex = i
        ...
        if sandboxindex != 0 and sys.argv[1] == 'off':
            ...
            data.insert(vmsvcindex + 1, (" " * pad) + useVMXSandbox)
        else:
            pass
